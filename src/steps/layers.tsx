...
export function Layers(props: {
    ...
    const [layers] = useState<LayersType>(() => {
        ...
        {names.map((name, index) => {
            ...
            return <React.Fragment key={index}>
                <div className="layer-container">
                    <Button
                        minimal={true}
                        icon={selected ? IconNames.CHEVRON_DOWN : IconNames.CHEVRON_RIGHT}
                        onClick={() => setSelectedLayer(selected ? "" : name)} />
                    {t("layer")}: &nbsp;&nbsp;
                    {selected ?
                        <input className="bp3-input" value={name} onChange={(e) => renameLayer(e, name)} /> :
                        name}
    ...
    if (buttons.length === 0) {
        ...
        buttonsComponent = (<div>
            ...
            <div className="touch-options" style={{ flexDirection: "column" }}>
                ...
                <div>
                    <img
                        ...
                        src={publicUrl + "/layers/all-buttons.jpg"} />
                </div>
            </div>
            <br />
            {buttons.map((button, index) => {
                ...
                return <React.Fragment key={index}>
                    <div className="touch-options">
                        <p><strong style={{ width: "4ch", display: "inline-block" }}>{index + 1}:</strong>
                            {t("button_symbol")}&nbsp;&nbsp;</p>
                        <input
                            className="bp3-input"
                            style={{ width: "6ch" }}
        ...
        gesturesComponent = (<div>
            ...
            {gestures.filter((g) => g.event !== "end:release").map((gesture, index) => {
                return <React.Fragment key={index}>
                    <div className="touch-options">
                        <p>{t("finger")}&nbsp;&nbsp;
                            <code
                                className="bp3-code fake-input"
                                style={{ width: "3ch" }}>{gesture.joystickId}</code>&nbsp;&nbsp;
                        </p>
                        <p>{t("gesture")}&nbsp;&nbsp;
                            <code
                                className="bp3-code fake-input"
                                style={{ width: "11ch" }}>{gesture.event}</code>&nbsp;&nbsp;
                        </p>
                        <p>{t("key")}</p>&nbsp;&nbsp;
                        <HTMLSelect minimal={false}
                            options={keyOptions}
            ...
            {Object.keys(releaseOnEnd).sort().map((finger, index) => {
                ...
                return <React.Fragment key={"finger-" + id}>
                    <div className="touch-options">
                        <Checkbox
                            checked={releaseOnEnd[id]}
                            onChange={(e) => onGestureChangeReleaseOnEnd(e, id as any)}>
                            {t("finger")}&nbsp;&nbsp;
                            <code className="bp3-code fake-input" style={{ width: "3ch" }}>{finger}</code>&nbsp;&nbsp;
                            {t("release_on_end")}
                        </Checkbox>
                    </div>
                </React.Fragment>;
            ...
            return <React.Fragment key={key}>
                <div className="touch-options">
                    ...
                    <Button
                        icon={IconNames.TRASH}
                        minimal={true}
                        onClick={() => onRemoveMapping(keyCode)}
                        style={{ alignSelf: "flex-start" }}></Button>
                </div>
            </React.Fragment>;
        })}
